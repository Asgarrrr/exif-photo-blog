    ...
    if (imageBase64) {
      if (
        ...
        const titleAndCaption = await generateOpenAiImageQuery(
          imageBase64,
          getAiImageQuery('title-and-caption', undefined, undefined, APP_LOCALE),
          isBatch,
        );
        if (titleAndCaption) {
        ...
        if (textFieldsToGenerate.includes('title')) {
          title = await generateOpenAiImageQuery(
            imageBase64,
            getAiImageQuery('title', undefined, existingTitle, APP_LOCALE),
            isBatch,
          );
        }
        if (textFieldsToGenerate.includes('caption')) {
          caption =  await generateOpenAiImageQuery(
            imageBase64,
            getAiImageQuery('caption', undefined, existingTitle, APP_LOCALE),
            isBatch,
          );
        }
      }
      ...
      if (textFieldsToGenerate.includes('tags')) {
        ...
        tags = await generateOpenAiImageQuery(
          imageBase64,
          getAiImageQuery('tags', existingTags, undefined, APP_LOCALE),
          isBatch,
        );
      }
      ...
      if (textFieldsToGenerate.includes('semantic')) {
        semanticDescription = await generateOpenAiImageQuery(
          imageBase64,
          getAiImageQuery('description-small', undefined, undefined, APP_LOCALE),
          isBatch,
        );
      }
    }
